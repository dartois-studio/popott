...
- Le logotype tient lieu de titre, avec le **numéro de version** en petit contre sa ligne
  de base — la même information qu'en tête des Réglages, lisible sans y aller.
...
- **Fiche plat** : chaque ligne d'ingrédient porte une pastille `opt` entre l'unité et la
  croix de suppression. Allumée, elle passe la ligne en ambre et ajoute la mention
  *optionnel* sous le nom.
...
- **Optionnel** : mention en ambre en tête de la ligne d'origine, devant les plats
  qui réclament l'article. L'article reste cochable comme les autres — c'est en magasin
  qu'on décide.
